...
## 10. 백테스트 튜닝 결과 (2026-06-08, 실데이터)

대상: 보유 DB(2,886종목·324,648행·2025-11-20~2026-06-05, 131거래일) 중 **유동성 상위 150종목**, 전략 `day-trading:breakout`, 실제 `BacktestService.runBacktest` 그대로 grid 실행(146종목 거래 발생). scale-out 활성 시 **TP1 매도비율** sweep:

| TP1 매도비율 | 평균수익% | 중앙수익% | 승률% | 수익종목% | 평균MDD% |
|---|---|---|---|---|---|
| baseline(전량청산) | 0.32 | 0.27 | 42.1 | 57.5 | 1.76 |
| 25% (러너 75%) | 0.69 | 0.58 | 51.3 | 63.0 | 1.82 |
| **33% (러너 67%) ← 채택** | 0.66 | 0.53 | 51.3 | **63.7** | 1.80 |
| 40% | 0.61 | 0.41 | 52.0 | 62.3 | 1.78 |
| 50% (Codex 초기기본) | 0.55 | 0.40 | 52.2 | 61.0 | 1.75 |

**결론**: ① scale-out 활성이 baseline 대비 평균수익 +106%·승률 +9%p·MDD 거의 불변으로 명확 우위. ② TP1 매도비율은 **낮을수록(러너 많이 남길수록) 수익↑**(단조), MDD는 미미하게↑. ③ **33% 채택** — 25%와 수익 거의 동일하면서 수익종목%↑·MDD↓로 균형 최적. → 4곳(libs/backend/controller/backtest) 기본값 50→33 통일(커밋 4477ea4).

**한계/주의**:
- 전(全)기간 단일 백테스트(walk-forward OOS 분리 아님) → 변형 간 **상대 비교는 견고**하나 절대 수익은 낙관적.
- 단일 전략(day-trading breakout). `mean-reversion:rsi`는 이 유동성 유니버스에서 거래 2종목뿐이라 결론 불가(=이 유니버스 주력은 breakout).
- 러너 강도(트레일링/익절폭) 변형은 영향 미미 → Codex 기본값(트레일링 3.5/2.5, 본전 4.0/1.0, 러너TP 6.0) 유지.
- **R_RISK_PCT는 백테스트로 튜닝 불가** — simulate가 매수수량에 R사이징을 반영하지 않음(라이브 전용 정책). 보수적 기본 0.5% 권장, 라이브/페이퍼로 검증.
- **토글은 여전히 OFF**. 실거래 적용 전 페이퍼(모의투자) 검증 권장.
